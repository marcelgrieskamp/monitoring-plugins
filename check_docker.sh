...
# Source: https://github.com/jryberg/nagios-plugin/blob/master/check_docker.sh
...
function ifexist () {
        local namedocker="$1"
        checkdocker=`sudo docker ps --filter "name=^/$namedocker\$" | grep $namedocker`
...
function getshortiddocker () {
        ...
        shortid=`sudo docker ps --filter "name=^/$namedocker\$" | grep $namedocker | awk '{print $1}'`
        echo "$shortid"
}
...
function getrxtxdocker () {
        ...
        #Get Container ID
        CID=`sudo docker inspect --format='{{.Id}}' $instanceid`
...
function getstats () {
        ...
        if [[ "$valid_version" == "1" ]]
        then
                currentstats=`sudo docker stats --no-stream --format "{{.Container}}|{{.$typestats}}" | grep $cid`
                echo "$currentstats" | awk '{split($0,a,"|"); print a[2]}'
        else
                case "$typestats" in
                      CPUPerc)
                        currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $2}'`
                        ;;
                      MemPerc)
                        currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $8}'`
                        ;;
                      *)
...
then
        ...
        then 
                ...
                then
                        statusdocker=`sudo docker ps --filter status=running | grep $shortid`
