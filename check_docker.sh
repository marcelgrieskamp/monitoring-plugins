...
# Source: https://github.com/jryberg/nagios-plugin
...
function dockerexist () {
        dckname="$1"
        container=`sudo docker stats --no-stream | grep $dckname`
...
function getstats () {
	...
	local cid="$2"
		case "$typestats" in
  		      CPUUsage)
			currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $3}'`
			;;
                      IORead)
			currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $11}'`
			;;
                      IOWrite)
                        currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $13}'`
                        ;;
                      Upload)
                        currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $10}'`
                        ;;
                      Download)
                        currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $8}'`
                        ;;
        	      MemUsage)
			currentstats=`sudo docker stats --no-stream | grep $cid | awk '{print $7}'`
			;;
		      *)
